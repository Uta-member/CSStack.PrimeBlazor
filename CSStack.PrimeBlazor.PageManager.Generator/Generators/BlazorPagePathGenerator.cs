...
namespace CSStack.PrimeBlazor.PageManager
{
    ...
    public class BlazorPagePathGenerator : IIncrementalGenerator
    {
        ...
        private static void Emit(
            ...
            ImmutableArray<(string Path, string PageName)> mappings)
        {
            ...
            foreach (var (path, pageName) in mappings)
            {
                sb.AppendLine(
                    $$"""
                        [Route("{{path}}")]
        ...
        /// </summary>
        private static ImmutableArray<(string Path, string PageName)> ExtractPathMappings(
            GeneratorAttributeSyntaxContext context)
        {
            var classSymbol = (INamedTypeSymbol)context.TargetSymbol;
            var mappings = ImmutableArray.CreateBuilder<(string Path, string PageName)>();
            ...
            foreach (var member in classSymbol.GetMembers())
            {
                ...
                {
                    mappings.Add((path, pageType.Name));
                }
            }
